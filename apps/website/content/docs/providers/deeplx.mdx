...
You can use the `{{token}}` placeholder in your baseURL to insert your API token at any position. This is useful when different DeepLX providers require tokens in different URL formats.

**Examples:**

1. **Token in path**: `https://api.deeplx.com/{{token}}/translate`
2. **Token as query parameter**: `https://api.deeplx.com/v1/translate?token={{token}}`
3. **Token in subdomain**: `https://{{token}}.api.deeplx.com/translate`
...
3. **Configure DeepLX**:
   ...
   - Use `{{token}}` placeholder if you need custom token placement
...
2. **Verify token placement**: Make sure `{{token}}` is in the correct position
3. **Test without token first**: Try with a free provider to isolate configuration issues

### Common Errors

- **"API key is required when using {{token}} placeholder"**: You specified `{{token}}` in the baseURL but didn't provide an API key
